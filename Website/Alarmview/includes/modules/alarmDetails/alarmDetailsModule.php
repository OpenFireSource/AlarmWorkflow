...
class alarmDetailsModule implements FahrzeughallenModule
{
	...
	public function update($objResponse, $arg)
	{
		...
		if($lastAlarm != $arg)
		{
			...
			if($zeile[Hinweis] != "" && $zeile["Mitteiler"] != "")
				$return .= "<td id=\"AlarmHinweis\" >" . $zeile[Hinweis] . "<br />" . $zeile["Mitteiler"] . "</td>";
			else if($zeile[Hinweis] != "" || $zeile["Mitteiler"] != "")
				$return .= "<td id=\"AlarmHinweis\">" . $zeile[Hinweis] . $zeile["Mitteiler"] . "</td>";
			else
				$return .= "<td style=\"text-align:center\"> <br /> </td>";
			$return .= "</tr><tr>";
			$return .= "<td id=\"AlarmOrt\">" . $zeile[Ort] . "<br />" . $zeile["Strasse"] . "<br />" .$zeile["Kreuzung"] .  "</td>";
			$return .= "<td id=\"AlarmObjekt\">" . $zeile["Fahrzeuge"] ."</td>";
			if($zeile[Objekt] == "")
			{
				$return .= "<br />";
			}
			...
			if($array[0] == "Planegg" || $array[0] == "Martinsried")
				$objResponse->script("directions.load(\"from: Planegg, Pasinger Strasse 24 to: ". $array[0] .", ". $strasseArray[0] ."\", { \"preserveViewport\": \"true\" });");
			else
				$objResponse->script("directions.load(\"from: Planegg, Pasinger Strasse 24 to: ". $array[0] .", ". $strasseArray[0] ."\");");				
		}
