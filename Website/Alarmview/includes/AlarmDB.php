...
class AlarmDB
{
	...
	{
		$result = $this->db->Execute("SELECT max( id ) FROM tb_einstaz ");
	...
	function getLastAlarm()
	{
		...
		$result = $this->db->Execute("SELECT Meldebild, Hinweis, Objekt, Einsatzplan, Ort, Strasse, Kreuzung, Einsatzort, Einsatznr, Mitteiler, Stichwort, Fahrzeuge FROM tb_einstaz WHERE ID = " . $val);
 		...
 		$zeile = array();
			...
			$zeile["Mitteiler"] = $result->fields[9];
